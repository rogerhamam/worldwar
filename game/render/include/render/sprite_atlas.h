...
public:
    ...
    // Registers/overrides a sprite's frame metadata at runtime (campaign custom
    // unit sprites). The PNG itself is still lazily loaded from asset_dir/sprites/
    // <name>.png on first draw; this just supplies the frame layout draw() needs.
    void register_sprite(const std::string& name, const Meta& m);

    // Same, but the PNG is loaded from an EXPLICIT file path instead of the
    // usual asset_dir/sprites|backgrounds/<name>.png lookup -- used for campaign
    // custom-unit sprites that live alongside the campaign JSON (e.g.
    // data/campaigns/sprites/<name>.png) rather than in the game's asset tree.
    void register_sprite(const std::string& name, const Meta& m, const std::string& file_path);
    ...
    bool draw_stretched(const std::string& name, const SDL_Rect& dst,
                         int frame = 0, bool flip = false, Uint8 alpha = 255);
...
private:
    ...
    // Explicit PNG paths for sprites registered via the file_path overload;
    // sheet() honours these before falling back to the asset_dir lookup.
    std::unordered_map<std::string, std::string> sprite_files_;
};
