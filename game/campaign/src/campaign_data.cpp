...
nlohmann::json map_event_to_json(const MapEvent& e) {
    ...
    auto& trips = j["trip_areas"] = nlohmann::json::array();
    for (auto& r : e.trip_areas) trips.push_back({{"tx", r.tx}, {"ty", r.ty}, {"tw", r.tw}, {"th", r.th}});
    if (e.trig_tx0 != -1) {
...
MapEvent map_event_from_json(const nlohmann::json& j) {
    ...
    if (j.contains("trip_areas")) {
        for (auto& rj : j.at("trip_areas")) {
            TileRect r;
            r.tx = rj.value("tx", 0);
            r.ty = rj.value("ty", 0);
            r.tw = rj.value("tw", 0);
            r.th = rj.value("th", 0);
            e.trip_areas.push_back(r);
        }
    }
...
nlohmann::json level_to_json(const Level& lvl) {
    ...
    auto& cu = j["custom_units"] = nlohmann::json::array();
    for (auto& c : lvl.custom_units) {
        cu.push_back({{"name", c.name}, {"base", c.base}, {"hp", c.hp}, {"attack", c.attack},
                      {"armor", c.armor}, {"sprite", c.sprite}, {"sprite_w", c.sprite_w},
                      {"sprite_h", c.sprite_h}});
    }
    return j;
}
...
Level level_from_json(const nlohmann::json& j) {
    ...
    if (j.contains("custom_units")) {
        for (auto& cj : j.at("custom_units")) {
            CustomUnit c;
            c.name = cj.value("name", "");
            c.base = cj.value("base", "muscateer");
            c.hp = cj.value("hp", 100.0);
            c.attack = cj.value("attack", 10.0);
            c.armor = cj.value("armor", 0.0);
            c.sprite = cj.value("sprite", "");
            c.sprite_w = cj.value("sprite_w", 0);
            c.sprite_h = cj.value("sprite_h", 0);
            lvl.custom_units.push_back(c);
        }
    }
    return lvl;
}
